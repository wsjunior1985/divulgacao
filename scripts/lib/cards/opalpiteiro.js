// opalpiteiro.js — layout do O Palpiteiro, no padrão aprovado do O Meu Álbum: foto
// neutra de celebração com troféu (IA no Canva, sem objeto de esporte, porque há
// bolões de qualquer tema), marca + nome, título em duas cores, três benefícios
// alinhados, faixa com "Grátis para jogar", um celular com a tela real e a pílula do site.
import { icone } from "../icones.js";
import { CSS_BASE, CSS_CELULAR, celular, conteudo, dominio, fontesCss, DISPLAYS, existe, logo, tela, url } from "./comum.js";

const VERDE = "#a855f7";
const AMARELO = "#fb923c";
const NAVY = "#0c0820";

export function opalpiteiro({ app, post, formato, L, A, variacao }) {
  const c = conteudo(app, post, 3);
  const barlow = DISPLAYS.barlow;
  const fundo = `assets/premium/${app.id}/fundo.jpg`;
  const [frente] = (c.telas ?? ["tela-app"]).map((n, i) => tela(app, n, variacao + i));
  const selo = c.selo ?? ["Grátis", "para jogar", "Premium para organizar"];
...
${fontesCss(barlow)}
:root { --verde:${VERDE}; --amarelo:${AMARELO}; --acento-rgb:168,85,247; }
${CSS_BASE}
...
.selo .ic { flex:none; width:52px; height:52px; border-radius:50%; display:grid; place-items:center; color:#fff;
  background:linear-gradient(135deg,#8b5cf6,#ec4899 60%,#fb923c); box-shadow:0 0 18px rgba(236,72,153,.45); }
...
.fig .ic { flex:none; width:60px; height:60px; border-radius:14px; display:grid; place-items:center; background:linear-gradient(135deg,#8b5cf6,#ec4899); color:#fff; }
...
.pe { position:absolute; left:46px; bottom:40px; right:46px; height:112px; display:flex; align-items:center; justify-content:center; gap:22px; padding:0 30px; border-radius:60px; z-index:5;
  background:linear-gradient(90deg,#7c3aed,#c026d3 55%,#f97316); color:#fff; box-shadow:0 16px 36px rgba(0,0,0,.45), 0 0 40px rgba(168,85,247,.4); }
...
<div class="marca"><img src="${url(logo(app))}"><b>O Palpiteiro</b></div>
...
<div class="esq">
  ...
  <div class="selo"><div class="ic">${icone("check")}</div><b>${selo[0]} ${selo[1]}</b><small>· ${selo[2]}</small></div>
</div>

<div class="pe"><div class="ic">${icone("globo")}</div><div><small>Crie seu bolão em</small><b>${dominio(app)}</b></div></div>
<div class="grao"></div>
